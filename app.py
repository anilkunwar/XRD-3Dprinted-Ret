...
# TAB 6 — PUBLICATION-QUALITY PLOT (UPDATED)
with tabs[6]:
    st.subheader("🖼️ Publication-Quality Plot (matplotlib)")
    st.caption("Generate journal-ready figures with fully customizable phase markers & legend")
    ...
    if "last_result" not in st.session_state:
        ...
        for i, ph in enumerate(phases):
            ...
            # ✅ PER-PHASE CUSTOMIZATION UI
            with st.expander(f"Settings for {ph}", expanded=True):
        ...
        with st.expander("🎨 Marker Shape Legend"):
            ...
            | `◆` Diamond | `d` | Sixth phase |
            | `×` Cross | `x` | Seventh phase |
            | `+` Plus | `+` | Eighth phase |
            | `*` Star | `*` | Ninth phase |
            """)
        plt.close(fig)
